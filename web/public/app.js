...
async function checkHealth() {
  try {
    ...
    updateHealthBadge('db', data.db?.toLowerCase() === 'ok');
    updateHealthBadge('cache', data.cache?.toLowerCase() === 'ok');
    updateHealthBadge('web', true); // Frontend tourne forcément si on arrive ici

    // Affiche le message global
    const allHealthy = (data.status === 'ok' || data.status === 'healthy') && data.db?.toLowerCase() === 'ok' && data.cache?.toLowerCase() === 'ok';
